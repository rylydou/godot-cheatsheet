...
func _process(delta: float) -> void:
	...
	find_child('TimeLabel').text = 'TIME: %sfps (u:%dms p:%dms)' % [fps, process_time, physics_process_time]
	find_child('MemLabel').text = 'MEM: %0.2fMB' % [mem_static/1_048_576]
	find_child('ObjLabel').text = 'OBJ: %s (%s nodes)' % [obj_count, node_count]
	find_child('RenderLabel').text = 'RENDER: %0.2fMB - %s %s' % [video_mem/1_048_576, draw_objs, draw_items]
